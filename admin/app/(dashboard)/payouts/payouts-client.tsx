...
export default function AdminPayoutsClient() {
    ...
    return (
        <div className="space-y-8">
            <h1 className="text-2xl font-bold tracking-tight text-white">Payout Requests</h1>

            <div className="rounded-2xl border border-white/5 bg-[#042f24] shadow-sm overflow-hidden">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {requests?.map((req) => (
                                <tr key={req.id} className="group hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="px-6 py-4 font-medium text-white">${req.amount}</td>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <Link
                                            href={`/payouts/${req.id}`}
                                            className="inline-flex items-center justify-center rounded-lg border border-white/10 bg-[#011d16] px-3 py-2 text-sm font-medium text-white shadow-sm hover:bg-white/10 focus:outline-none transition-all"
                                        >
                                            Process
