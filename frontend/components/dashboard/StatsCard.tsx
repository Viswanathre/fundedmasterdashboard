...
export default function StatsCard({ title, value, change, isPositive, icon: Icon, color = "blue" }: StatsCardProps) {
    const colorStyles = {
        blue: "bg-[#d9e838]/10 text-[#d9e838] border-[#d9e838]/20",
    ...
    return (
        <motion.div
            ...
            className="bg-[#042f24] rounded-xl border border-white/5 p-6 relative overflow-hidden group hover:border-white/10 transition-all font-sans"
        >
            <div className="flex justify-between items-start mb-6 relative z-10">
                <div className="flex items-center gap-3">
                    {Icon && (
                        <div className={cn("p-2 rounded-lg border", colorStyles[color === "blue" ? "blue" : color])}>
                            <Icon size={18} />
                        </div>
                    )}
                    <h3 className="text-emerald-500/60 font-medium text-sm tracking-wide">{title}</h3>
                </div>
                ...
                <div>
                    ...
                    {change && (
                        <div className="flex items-center gap-2 mt-2">
                            ...
                            <span className="text-emerald-500/40 text-xs">vs last month</span>
                        </div>
                    )}
                </div>
            </div>
